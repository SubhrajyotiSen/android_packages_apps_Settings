...
public class ColorPickerDialog extends Dialog implements
    ...
    private final int mOwnColor;
    private int mNewColorValue;

    private static final int PALETTE_OWN  = 0;
    ...
    private void setUp() {
        ...
        if (mAndroidColor != 0x00000000 && mOwnColor != 0x00000000) {
    ...
    public boolean onMenuItemClick(MenuItem item) {
        if (item.getItemId() == R.id.palette_own) {
            setPalette(PALETTE_OWN);
            mColorTransitionAnimator.start();
            return true;
            ...
            return true;
        } else if (item.getItemId() == R.id.reset_own) {
            mColorPicker.setColor(mOwnColor, true);
            return true;
        }
        return false;
    }
    ...
    private int getPalette() {
        return Settings.System.getInt(mResolver,
                Settings.System.COLOR_PICKER_PALETTE, PALETTE_OWN);
    }
    ...
    private int getPanelViewButtonColor(int pallete, int index) {
        TypedArray ta;
        if (pallete == PALETTE_OWN) {
            ta = mResources.obtainTypedArray(R.array.color_picker_own_palette);
